...
`omnivoice` is available as a second native TTS engine through
`omnivoice.cpp`. It uses the same low-level subprocess/WAV/PCM path as
Qwen3TTS, but its CLI arguments and clone pre-encoding stay inside the
OmniVoice adapter.

Build the lab runtime and download the recommended models:

```bash
./scripts/omnivoice/setup_omnivoice_lab.sh
```

The default model pair is `omnivoice-base-Q8_0.gguf` plus
`omnivoice-tokenizer-F32.gguf`. The base model keeps Q8_0 for voice quality;
the tokenizer keeps F32 to avoid extra RVQ noise.

Benchmark or smoke-test the provider:

```bash
./scripts/omnivoice/bench_omnivoice_lab.sh
./scripts/omnivoice/smoke_omnivoice_provider.sh
```

OmniVoice modes in the console:

- Auto: automatic voice from the base model.
- Design: `--instruct` voice labels. OmniVoice expects supported items such as
  `female, young adult, chinese accent, moderate pitch`; it does not accept
  arbitrary prose the way Qwen3TTS VoiceDesign does.
- Clone: shared voice profile references plus OmniVoice-specific `.rvq`
  generated by `omnivoice-codec`.

For pseudo-streaming, the adapter uses `omnivoice-tts -o -` and decodes the WAV
stream from stdout. File-output synthesis remains the fallback path used by the
normal `synthesize` call.
...
2. Qwen3TTS latency improvements.
   ...
   - Keep `sherpa_kokoro` as the quick fallback for low-resource or broken-GPU
     sessions.
   - Keep OmniVoice as the alternate native engine for auto/design/clone voices.
